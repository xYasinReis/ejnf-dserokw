...
exports.run = async(client, message, args) => {
    ...
    let sebep = args.slice(1).join(' ') || "Belirtilmemiş."
     ...
     if(user.id === message.author.id) return message.channel.send('<a:unlem:814527254029402193> ``Kendini Kickleyemezsin``')
     if(user.id === client.user.id) return message.channel.send('<a:unlem:814527254029402193> ``Botu Kickleyemezsin``')
  if(user.id === message.guild.ownerID) return message.channel.send('<a:unlem:814527254029402193> ``Sunucu sahibini Kickleyemessin Zaten Yetkim Yetmez :)``')
